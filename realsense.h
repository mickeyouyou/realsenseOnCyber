...
class RealSense {
 public:
  static rs2::device getDevice() {
    ...
    AINFO << "Found the following devices:\n";
  ...
  static void printDeviceInformation(const rs2::device& dev) {
    ...
    AINFO << "Device information: ";
    // The following code shows how to enumerate all of the RS2_CAMERA_INFO
    // Note that all enum types in the SDK start with the value of zero and end
    // at the "*_COUNT" value
  ...
  static rs2::sensor getSensorFromDevice(const rs2::device& dev) {
    ...
    AINFO << "Device consists of " << sensors.size() << " sensors:\n";
    int index = 0;
  ...
  static rs2_option getSensorOption(const rs2::sensor& sensor) {
    ...
    AINFO << "Sensor supports the following options:\n";
  ...
  static float getDepthUnits(const rs2::sensor& sensor) {
    ...
    // value should be multiplied by the sensor's depth scale Here is the way to
    // grab this scale value for a "depth" sensor:
  ...
  static void get_field_of_view(const rs2::stream_profile& stream) {
    ...
    // that produces the data has a lens and thus has properties such
    //  as a focal point, distortion, and principal point.
    // To get these intrinsics parameters, we need to take a stream and first
    // check if it is a video stream
    if (auto video_stream = stream.as<rs2::video_stream_profile>()) {
      try {
        // If the stream is indeed a video stream, we can now simply call
        // get_intrinsics()
        ...
        AINFO << "Principal Point         : " << principal_point.first << ", "
              << principal_point.second;
        AINFO << "Focal Length            : " << focal_length.first << ", "
              << focal_length.second;
        AINFO << "Distortion Model        : " << model;
        AINFO << "Distortion Coefficients : [" << intrinsics.coeffs[0] << ","
              << intrinsics.coeffs[1] << "," << intrinsics.coeffs[2] << ","
              << intrinsics.coeffs[3] << "," << intrinsics.coeffs[4] << "]";
      } catch (const std::exception& e) {
        AERROR << "Failed to get intrinsics for the given stream. " << e.what();
      }
    } else if (auto motion_stream = stream.as<rs2::motion_stream_profile>()) {
      try {
        // If the stream is indeed a motion stream, we can now simply call
        // get_motion_intrinsics()
        rs2_motion_device_intrinsic intrinsics =
            motion_stream.get_motion_intrinsics();

        AINFO << " Scale X      cross axis      cross axis  Bias X \n";
        AINFO << " cross axis    Scale Y        cross axis  Bias Y  \n";
        AINFO << " cross axis    cross axis     Scale Z     Bias Z  \n";
        for (int i = 0; i < 3; i++) {
          for (int j = 0; j < 4; j++) {
            AINFO << intrinsics.data[i][j] << "    ";
          }
          AINFO << "\n";
        }

        AINFO << "Variance of noise for X, Y, Z axis \n";
        for (int i = 0; i < 3; i++)
          AINFO << intrinsics.noise_variances[i] << " ";
        AINFO << "\n";

        AINFO << "Variance of bias for X, Y, Z axis \n";
        for (int i = 0; i < 3; i++)
          AINFO << intrinsics.bias_variances[i] << " ";
        AINFO << "\n";
      } catch (const std::exception& e) {
        AERROR << "Failed to get intrinsics for the given stream. " << e.what();
      }
  ...
  static void getExtrinsics(const rs2::stream_profile& from_stream,
    ...
    try {
      // Given two streams, use the get_extrinsics_to() function to get the
      // transformation from the stream to the other stream
      rs2_extrinsics extrinsics = from_stream.get_extrinsics_to(to_stream);
      AINFO << "Translation Vector : [" << extrinsics.translation[0] << ","
            << extrinsics.translation[1] << "," << extrinsics.translation[2]
            << "]\n";
      AINFO << "Rotation Matrix    : [" << extrinsics.rotation[0] << ","
            << extrinsics.rotation[3] << "," << extrinsics.rotation[6] << "]\n";
      AINFO << "                   : [" << extrinsics.rotation[1] << ","
            << extrinsics.rotation[4] << "," << extrinsics.rotation[7] << "]\n";
      AINFO << "                   : [" << extrinsics.rotation[2] << ","
            << extrinsics.rotation[5] << "," << extrinsics.rotation[8] << "]";
    } catch (const std::exception& e) {
      AERROR << "Failed to get extrinsics for the given streams. " << e.what();
    }
  }
  ...
  static void changeSensorOption(const rs2::sensor& sensor,
    ...
    // First, verify that the sensor actually supports this option
    if (!sensor.supports(option_type)) {
      AERROR << "This option is not supported by this sensor";
      return;
    }

    // Each option provides its rs2::option_range to provide information on how
    // it can be changed To get the supported range of an option we do the
    // following:

    AINFO << "Supported range for option " << option_type << ":";
    ...
    AINFO << "  Min Value     : " << minimum_supported_value << std::endl;
    AINFO << "  Max Value     : " << maximum_supported_value << std::endl;
    AINFO << "  Default Value : " << default_value << std::endl;
    ...
    if (change_option) {
      ...
      // To set an option to a different value, we can call set_option with a
      // new value
      try {
        sensor.set_option(option_type, requested_value);
      } catch (const rs2::error& e) {
        // Some options can only be set while the camera is streaming,
        // and generally the hardware might fail so it is good practice to catch
        // exceptions from set_option
        AERROR << "Failed to set option " << option_type << ". (" << e.what()
               << ")";
      }
    }
  }
  ...
  static rs2::stream_profile chooseStreamingProfile(const rs2::sensor& sensor) {
    ...
    // Usually a sensor provides one or more streams which are identifiable by
    // their stream_type and stream_index Each of these streams can have several
    // profiles (e.g FHD/HHD/VGA/QVGA resolution, or 90/60/30 fps, etc..)
    // The following code shows how to go over a sensor's stream profiles, and
    // group the profiles by streams.
    std::map<std::pair<rs2_stream, int>, int> unique_streams;
    for (auto&& sp : stream_profiles) {
      unique_streams[std::make_pair(sp.stream_type(), sp.stream_index())]++;
    }
    AINFO << "Sensor consists of " << unique_streams.size()
          << " streams: " << std::endl;
    for (size_t i = 0; i < unique_streams.size(); i++) {
      auto it = unique_streams.begin();
      std::advance(it, i);
      AINFO << "  - " << it->first.first << " #" << it->first.second;
    }

    // Next, we go over all the stream profiles and print the details of each
    // one
    AINFO << "Sensor provides the following stream profiles:" << std::endl;
    int profile_num = 0;
    for (rs2::stream_profile stream_profile : stream_profiles) {
      ...
      //    * rs2_stream::RS2_STREAM_INFRARED describes a stream of infrared
      //    images
                                                 ...
                                                 // tested is of the type given
      {
        ...
        AINFO << " (Video Stream: " << video_stream_profile.format() << " "
    ...
    uint32_t selected_profile_index = 1;
    // get_user_selection("Please select the desired streaming profile: ");
  ...
  static void startStreamingProfile(const rs2::sensor& sensor,
    ...
    // Open can be called with a single profile, or with a collection of
    // profiles Calling open() tries to get exclusive access to the sensor.
    ...
    // In order to begin getting data from the sensor, we need to register a
    // callback to handle frames (data) To register a callback, the sensor's
    // start() method should be invoked. The start() method takes any type of
    // callable object that takes a frame as its parameter NOTE:
    ...
    // To prevent this thread from returning, we block it using the helper
    // wait() function
    ...
    // To complete the stop operation, and release access of the device, we need
    // to call close() per sensor
    sensor.close();
  }
};